...
func _initialize() -> void:
	...
	_expect(main.get_node("Toolbar/StarButton").visible, "factory tab should show the alternate Star template with the other plans")
	...
	_expect("魔力 40/100" in scout_plan_tooltip.context and "/32秒" in scout_plan_tooltip.context and "初着" in scout_plan_tooltip.context, "template hover should compare its current-run mana, production, and opening timing without applying it")
	...
	_expect(main.get_node("Toolbar/StarButton").forecast_first_tick == main._first_production_tick(main.factory_board.plan_production_snapshot(MvpContent.PLAN_STELLAR)), "plan comparison should expose first arrival through the same cached non-destructive forecast")
	_expect(main.get_node("Toolbar/StarButton").forecast_first_tick > 0 and main.get_node("Toolbar/SentinelButton").forecast_first_tick > 0, "completed template Glyphs should carry a visible first-arrival marker")
	_expect(main.get_node("Toolbar/EmptyButton").forecast_first_tick < 0, "manual wiring should not invent an arrival marker before its graph exists")
	...
	_expect(main.pause_button.disabled, "an incomplete hand-built factory should not present battle start as available")
	main._on_main_action()
	_expect("戦闘を開始できません" in main.status_label.text, "keyboard start should still explain why an incomplete factory cannot run")
	...
	_expect(not main.pause_button.disabled, "battle start should become available as soon as the factory is valid")
	_expect(main.status_label.text == "", "repairing the factory should clear the stale start error")
	...
	_expect(main.pause_button.disabled, "breaking the required connection should disable battle start again")
	...
	_expect(not main.pause_button.disabled, "a complete template should keep battle start available")
	...
	var meaning_palette_tooltip = main.get_node("FactoryPalette/MeaningButton")._make_custom_tooltip("")
	_expect("目・十字・的・星・方位" in meaning_palette_tooltip.context, "meaning-source hover should disclose every registered Glyph choice")
	_expect(meaning_palette_tooltip._primitive_name(&"circle") == "丸", "meaning Glyph details should localize the basic shape vocabulary")
	meaning_palette_tooltip.free()
	...
	_expect(main.get_node("FactoryPalette/RotateButton").goal_state == &"missing" and main.get_node("FactoryPalette/ColorButton").goal_state == &"missing", "target processors absent from the current scout factory should use the missing marker")
	main.get_node("FactoryPalette/RotateButton").set_availability(false, &"mana")
	main._refresh_factory_goal_tools()
	_expect(main.get_node("FactoryPalette/RotateButton").goal_state == &"blocked", "missing target equipment should distinguish an unavailable addition")
	main._refresh_factory_palette_state()
	_expect(main.get_node("FactoryPalette/RotateButton").goal_state == &"missing", "restored availability should return a blocked target tool to missing")
	...
	var marked_unit = main.battle_board.simulation.units.back()
	var marked_glyph = main.battle_board.unit_sigil_glyph(marked_unit)
	_expect(marked_glyph.canonical_serialization() == MeaningGlyphsModel.glyph(MeaningGlyphsModel.STAR).canonical_serialization(), "battle units should retain the exact meaning sigil that produced their combat variant")
	marked_glyph.components[0].position = Vector2i(99, 99)
	_expect(main.battle_board.unit_sigil_glyph(marked_unit).canonical_serialization() == MeaningGlyphsModel.glyph(MeaningGlyphsModel.STAR).canonical_serialization(), "battle unit sigil inspection should not expose the shared render cache for mutation")
	main.battle_board.simulation.units.pop_back()
	_expect(not main.get_node("Toolbar/StarButton").visible, "battle tab should hide the alternate Star template with the other factory plans")
	...
	_expect("襲撃兵 ×1" in main.threat_label.text and "推奨" not in main.threat_label.text, "enemy forecast should expose the threat without prescribing a factory answer")
	_expect("編成警告 60s: 群体兵" in main.threat_label.text, "battle should warn major wave changes sixty seconds ahead")
	...
	_expect(main.battle_board.wave_status_text() == "混成の道 // 襲撃兵", "battlefield should identify both route and upcoming wave")
	...
	_expect(main.battle_result_sigil_strip.metrics[&"kills"] == main.battle_board.simulation.player_kills and main.battle_result_sigil_strip.metrics[&"time"] is String, "victory should retain factual battle metrics in the visual result strip")
	_expect(main.battle_result_sigil_strip.entries[0][&"produced"] == 3, "victory Glyph metrics should preserve factory production without a duplicate sentence")
	_expect(main.battle_result_sigil_strip.metrics[&"pauses"] == 2, "victory metrics should preserve committed and discarded time stops")
	var pause_metric_center: Vector2 = main.battle_result_sigil_strip.metric_rect(2).get_center()
	_expect("時間停止 // 2回" in main.battle_result_sigil_strip._get_tooltip(pause_metric_center), "visual result metrics should explain their compact symbols on hover")
	_expect(main.phase_body.text == "", "victory should not duplicate visual sigil and battle metrics in a permanent result paragraph")
	...
	first_reward.set_level(first_reward.MAX_LEVEL)

...
func _test_factory_board_exposes_visible_work_in_progress_glyphs() -> void:
	...
	_expect(board.line_goal_progress_level(&"line_1") == 1, "raw material path should show the first stage of target progress")
	_expect(board.line_goal_progress_level(&"line_2") == 2, "rotated path should show increased target progress")
	_expect(board.line_goal_progress_level(&"line_3") == 3, "completed path should show full target progress")

...
func _test_sigil_ghost_tracks_plan_recipe() -> void:
	...
	_expect("取得済みの別シジル" in ghost.candidate_context(), "alternate registered output should explain its neutral state only on demand")
	...
	_expect("未登録" in ghost.candidate_context(), "red mismatch should remain reserved for an unregistered output")
